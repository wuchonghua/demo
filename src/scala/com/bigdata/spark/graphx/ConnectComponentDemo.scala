...
object ConnectComponentDemo extends Serializable {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val peopleRDD: RDD[(VertexId, People)] = sc.textFile("datas\\people.csv")
      .map(_.split(","))
      .map(x => (x(0).toLong, People(x(1), x(2).toInt)))

    val linksRDD: RDD[Edge[String]] = sc.textFile("datas\\link.csv")
      .map { x =>
        val row = x.split(",")
        Edge(row(0).toInt, row(1).toInt, row(2))
      }
    val graph=Graph(peopleRDD,linksRDD)
    val mincc: Graph[VertexId, String] = graph.stronglyConnectedComponents(20)
//    val mincc: Graph[VertexId, String] = graph.connectedComponents()
    mincc.vertices.collect().foreach(x => println(x + "-------1"))

    val newGraph: Graph[(VertexId, String, PartitionID), String] = mincc.outerJoinVertices(peopleRDD)((id,mincc,p)=>(mincc,p.get.name,p.get.age))
